...
- **Living regression checklist — automatic parts and their off switches** (`=0` turns one off; `.agents/CHECKLIST.md`, `docs/plans/regression-checklist-v3.md`):
  ...
  | `TEST_RUN_LOCK_WAIT_S` (seconds, default 900 from the CLI; the Stop hook passes 120 unless set) | how long the gate waits for `.claude/audit-gate/test_run.lock`, the per-project lock the gate, stale re-run and nightly hold so two runs never share one build dir; past it the tests are BUSY (exit 4, JSON `"busy": true`), never run side by side — the Stop hook says "another run holds the lock, run again later" and caches nothing. Stale re-run and nightly wait at most their suite timeout instead; once they hold the lock the suite gets its whole timeout, so a recorded TIMEOUT is always the suite's own (a job may take up to twice its timeout) | `post-fix-gate`, `hooks/regression_gate.sh`; `scripts/stale_rerun.py`, nightly (lock only) |
...
### 7.1 One Developer, One Branch — and a Worktree only for Parallel Agents

**Default: one branch.** A solo developer works on one branch (`main`/`trunk`); every extra branch, worktree or remote branch is code that can drift. GeelyEx2, 2026-09-26: `git push origin <sha>:main` of commits the local `main` did not hold left local `main` two commits behind origin, and worktree branches piled up. So:
- **Git guard** (`hooks/block-dangerous-git.sh`, every platform with hooks): `git checkout -b|-t|--orphan`, `git switch -c|-t|--orphan`, `git branch <new>` (also `-c` copy, `-f`), `git worktree add`, a push to a branch the local repo does not have, and a push of `<src>:<dst>` whose `<src>` the local `<dst>` does not contain (`git merge-base --is-ancestor`, in the repo named by `-C`, the last `cd`, or the hook's `cwd`) are blocked (exit 2) with the fix. A `<src>:<dst>` push that cannot be checked (unknown directory such as `cd "$R"`, a `$(…)` source, a git error or the 5 s timeout) is blocked too — push the branch by name instead; the branch-creation checks never block on a git error. A `$(…)` / backtick span counts as one word, so a commit message written with `$(cat <<'EOF' …)` is not read as commands; a `cd` inside `( … )` does not move the commands after it; shell redirections (`2>/dev/null`) are not branch names. Listing, `-d`, `-m`, `-u`, tags (`v1.0:v1.0`), `refs/for/…` and plain `git push origin <branch>` pass. Not caught: `git checkout <name>` that git turns into a new tracking branch, git run through a variable (`$g push …`), and `agent-kit worktree add` (it is the path below).
- **A branch the user asked for:** put `DEVKIT_ALLOW_BRANCH=1` in front of the git command (or `export` it). Never set it on your own. It lifts the new-branch checks only — never a push of `<src>:<dst>` that the local `<dst>` does not hold.
- **Session start** (`hooks/session_context.sh`) fetches the upstream (6 s at most; `SESSION_FETCH=0` skips it) and names, with the command that fixes each: the branch behind (`git pull --ff-only`), ahead, or both (`git pull --no-rebase`, then push); a detached HEAD or a side branch; leftover worktrees; extra local branches (merged → `git branch -d …`; unmerged → merge back or ask). `main`, `master`, `trunk`, `develop`, `release/*`, `hotfix/*` are never called extra.
- **Push:** bring origin into the local branch as its own command, check its exit code (a pipe hides it), then `git push origin <branch>`.

**Parallel agents** are the exception. Two agents editing one working tree overwrite each other's files, mix their diffs in `git status` and break each other's builds mid-run. When two or more agents change code at the same time, each gets its own worktree and branch — and in the same turn the work is brought back, the worktree is removed and the branch deleted (`git branch -d`).
...
- **Worktree guard** (`hooks/worktree_guard.sh`, Claude Code, PreToolUse on Bash and Edit|Write|MultiEdit|NotebookEdit): once a session or subagent has a worktree, a write into the main checkout of the same repo is blocked (exit 2) — an Edit there, a redirection, a `cp`/`mv`/`rm`/`sed -i`/`dd of=` target, a git write (`commit`, `checkout`, `stash push/pop`, …) or a build tool run there, also through an interpreter (`sh ./gradlew …`, `python3 -m pytest …`). Reads of the main checkout, `git stash list`, `python3 -c …` and copies to `host:` pass, and so does state that exists only in the main checkout: `.claude/agent-memory/` and `.claude/audit-gate/` unless the file is tracked, `.agents/local/memory/` where git ignores it, and its `claude-auto/` (auto-memory) always; a tracked or not-ignored file there (`bugs/*.md`) is blocked. A worktree is declared by `DEVKIT_WORKTREE=<path>`, by the harness (`isolation: "worktree"`: the subagent's `worktreePath`, or its transcript's first `cwd`), by the session having STARTED in a worktree (the first `cwd` of its transcript), or by the session having ENTERED one with the Claude Code tool `EnterWorktree` (the last successful `EnterWorktree`/`ExitWorktree` call in its transcript is an Enter; a failed call changes nothing, `ExitWorktree` ends it) — never by a later `cd`: the leader may look into a worktree, then edit, `git merge` or `git apply` in the main checkout. `claude --resume` from another directory is not seen. A subagent whose prompt only names a worktree made by `agent-kit worktree add` gets a warning, not a block. Off: `WORKTREE_GUARD=0` (logged in `.claude/audit-gate/worktree_guard.log`).
