...
- **Những gì bị chặn:** lệnh git phá hủy (`git push --force`, `git reset --hard`, kể cả dạng bọc `(…)`, `timeout`, `sudo -u`, alias), lệnh thiết bị nguy hiểm (`adb remount`, `fastboot flash`, `dd of=/dev/…`), sửa file chưa đọc, sửa file nhạy cảm khi chưa review bảo mật, và agent làm việc trong git worktree mà ghi vào checkout chính (`worktree_guard.sh`).
...
- **Gate không chụp ảnh.** Exit code không gồm PNG. Agent vẫn phải gắn ảnh của lượt này theo `rules/essentials.md` ("Every prompt") trước khi nói XONG. Trên Claude Code, Stop hook `proof_gate.sh` chặn câu trả lời mở bằng XONG trừ khi lượt đó có đủ: cổng `--run-tests --full` exit 0 trên đúng code hiện tại (cổng ghi biên nhận ở `.git/postfix-gate/full_pass.json`, sửa code sau đó là mất hiệu lực) và nêu `reports/proof-<yyyyMMdd-HHmmss>.png` thật (PNG, > 8 KB, chụp trong lượt). `regression_gate.sh` lúc dừng vẫn chỉ chạy nhanh test bị ảnh hưởng; tắt bằng `PROOF_GATE=0`. Chỉ nhắc thêm: DESIGN.md/a11y, RED→GREEN, Immutable Guards, OpenCodeReview.
...
- **Không rollback âm thầm.** Mỗi lần DevKit ghi `.agents/regression_status.json` được ghi sổ ở `.agents/regression_journal/` (thời điểm, tool + pid, hash nội dung; 500 dòng và 10 snapshot gần nhất, git-ignored). File bị ghi đè ngoài DevKit và mất nội dung (ví dụ `git show HEAD:… > …` xoá red_proof hay unlink) thì mỗi lần load, context SessionStart và `agent-kit health` đều cảnh báo, kèm đường dẫn snapshot tốt cuối, cho tới khi `agent-kit checklist restore` gộp snapshot đó lại. Gộp chứ không chép đè: red_proof và kết quả mới hơn theo thời gian thắng, link = hợp hai bên trừ các unlink đã ghi, dòng thêm sau vẫn giữ. `restore --list` liệt kê snapshot, `restore <snapshot>` gộp một bản chọn, `restore --dismiss` giữ một lần reset cố ý.
...
| 4. Xong (trước khi nói "xong") | `/done`, rồi `/audit-gate` | `verification-before-completion` + post-fix gate | `post-fix-gate.py --run-tests --full` (exit 0) và PNG nghiệm thu của lượt này, trừ khi thay đổi chắc chắn không lên màn hình (`rules/essentials.md` bước 4) | Gate không tự chụp ảnh và không chứng minh RED→GREEN |
