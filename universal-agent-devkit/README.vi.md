...
- **Gate không chụp ảnh.** Exit code không gồm PNG. Agent vẫn phải gắn ảnh của lượt này theo `rules/essentials.md` ("Every prompt") trước khi nói XONG. Trên Claude Code, Stop hook `proof_gate.sh` chặn câu trả lời mở bằng XONG khi không nêu `reports/proof-<yyyyMMdd-HHmmss>.png` thật (PNG, > 8 KB, chụp trong lượt); tắt bằng `PROOF_GATE=0`. Chỉ nhắc thêm: DESIGN.md/a11y, RED→GREEN, Immutable Guards, OpenCodeReview.
