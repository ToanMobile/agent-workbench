...
3. **Bằng chứng nghiệm thu & Báo cáo Tự nhiên Súc tích (BẮT BUỘC):**
   - MỌI báo cáo kiểm thử / nghiệm thu BẮT BUỘC có **bằng chứng thật của trạng thái THÀNH CÔNG (Pass / Success State)**. Luật đứng nằm ở `rules/essentials.md` mục "Every prompt": cùng lượt phải có `post-fix-gate.py --run-tests --full` exit 0 và, trừ khi thay đổi chắc chắn không lên màn hình (essentials bước 4), một PNG chụp trong lượt (`reports/proof-<yyyyMMdd-HHmmss>.png`), gắn vào câu trả lời. Serial trong `.adb-denylist` hoặc bị luật dự án cấm thì không chụp. Ảnh chụp bằng `python3 .agents/devkit/bin/proof-capture.py`: serial đã khai mà offline thì lệnh không screencap vào địa chỉ đó và không đổi sang máy khác đang cắm — nó mở AVD rồi chụp. Lệnh thoát khác 0 thì CHƯA XONG. Cấm vẽ ảnh hoặc lấy XML test thay ảnh. Kèm **Báo Cáo Nghiệm Thu 4 Mục**, mỗi mục 1–2 dòng (vừa giới hạn 12 dòng ở §13):
...
- **Lười về lời giải, không lười về đọc và kiểm chứng:** "Lười" ở đây là viết ít dòng hơn. Nó không cho phép đọc ít hơn, test ít hơn hay bỏ gate (§5 và `essentials.md` vẫn nguyên hiệu lực).
- **Thang 7 bậc (dừng ở bậc đầu tiên dùng được), chỉ leo sau khi đã hiểu thay đổi:** (1) có cần tồn tại không (YAGNI) → (2) codebase đã có → tái sử dụng → (3) standard library → (4) tính năng native của nền tảng (`<input type="date">` thay thư viện picker, CSS thay JS, constraint DB thay code app) → (5) dependency đã cài → (6) một dòng → (7) mới viết phần tối thiểu chạy được. Hai cách stdlib cùng cỡ → chọn cách đúng ở edge case.
- **Không viết thứ không ai yêu cầu:** interface chỉ có một implementation, factory chỉ ra một loại, config cho giá trị không bao giờ đổi, scaffolding "để sau", file mới khi file cũ chứa được.
- **Bug sửa ở gốc, một lần:** grep mọi caller của hàm sắp sửa; một guard ở hàm dùng chung nhỏ hơn một guard ở mỗi caller, và vá riêng đường ticket nêu thì các caller anh em vẫn lỗi.
