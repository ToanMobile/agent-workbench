...
# templates/claude_settings.json: it only understands .kt/.java bursts and warns
# on every PostToolUse edit, which is noise outside a Kotlin/Java project. Wire it
# yourself under PostToolUse "Edit|Write|NotebookEdit" to enforce W3 timing.
#
...
if [ "${REVIEW_TIMING_GUARD:-1}" = "0" ]; then
  echo "[$(date +%Y-%m-%dT%H:%M:%S)] REVIEW_TIMING_GUARD=0 — gate bypassed" >> "${LOG_DIR}/review_timing_guard.log" 2>/dev/null
  exit 0
fi
