...
- **`run_impacted.sh` chọn cả test gọi script qua lệnh**: ngoài tên file (`githooks.sh`), test nhắc tên không đuôi như một từ (`agent-kit githooks install`) cũng được chọn. Sự cố gốc (2026-09-26): luật subject mới làm `test_githooks` đỏ (commit subject `clean`), cổng không chạy nó và commit `86f7ddc` lên origin với test đỏ; các subject trong `test_githooks.sh` nay là câu rõ nghĩa, để chỉ pre-commit quyết định kết quả các ca chặn. Test: `tests/test_run_impacted.sh`.
- **Pre-commit chạy các suite hồi quy nhanh mà file đã stage chạm tới**, không chỉ kiểm tĩnh. Trước đây test chỉ chạy trong phiên agent (cổng Stop), nên commit gõ tay với subject rõ ràng vẫn đưa được code làm đỏ checklist (Goods 2026-09-25). Chạy: suite của matrix có `watch_files` khớp file đã stage, không phải Gradle/Unity/xcodebuild, lần chạy ghi trong checklist ≤ 30 s (`DEVKIT_PRECOMMIT_MAX_S`), tổng ≤ 120 s (`DEVKIT_PRECOMMIT_BUDGET_S`). Suite ĐỎ chặn commit và in 15 dòng cuối; suite quá 60 s (`DEVKIT_PRECOMMIT_TEST_TIMEOUT`) bị dừng và chỉ cảnh báo; suite bị bỏ qua được nêu tên kèm lý do. `DEVKIT_PRECOMMIT_TESTS=all` chạy cả suite nặng, `=0` tắt. Tính trên matrix + checklist thật: GeelyEx2 chạy tới 16 script Python/Go ≤ 2,2 s mỗi cái, OfficeReader 2, Goods 1, workbench chỉ health (bỏ 5 suite 47–158 s); Gradle/Unity để cổng Stop và nightly — nên ca Unity của Goods vẫn không bị pre-commit bắt nếu không bật `=all`. `ponytail:` test chạy trên cây làm việc (gồm cả sửa chưa stage), không phải riêng nội dung đã stage. Test: `tests/test_commit_hygiene.sh`.
- **Suite DevKit của cổng chạy song song**: `tests/run_impacted.sh` (suite `REG-DK-ALL-01`) chạy `DEVKIT_TEST_JOBS` test cùng lúc (mặc định 4), in kết quả theo đúng thứ tự danh sách. Hai test đo thời gian thực (`test_budgets`, phần fetch có trần 6 s của `test_session_context`) chạy riêng sau cùng. Ngày 26/09 suite mất 740 s trên giới hạn 900 s của cổng; đo trên cùng danh sách 31 test: tuần tự 445 s, song song 165 s, 31/31 XANH. Chưa làm: lần `test_budgets` đỏ 167 ms trong cổng không tái hiện được, kể cả khi dựng tải CPU (load average 102 vẫn chỉ 85 ms); ngân sách chưa đổi. Test: `tests/test_run_impacted.sh`.
- **Commit sửa code không được có subject mơ hồ** (hook `commit-msg`): commit có file code nguồn của profile bị chặn khi subject là `<type>: <1 từ>`, dưới 3 từ nếu không theo dạng `<type>(scope):`, hoặc là chữ đệm (`update`, `wip`, `fix`, `no message`, `sửa lỗi`…). Mọi type đều nhận (`data(voice):`, `tools(vhal):`), và subject dạng câu ≥ 3 từ cũng nhận (script release của GeelyEx2 commit `Release artifacts (dev): …`). Subject do git sinh được bỏ qua: `Merge `, `Revert `, `fixup!`/`squash!`/`amend!`, `Initial commit`, `Squashed commit of the following:`. Ký tự BOM đầu message được bỏ. Subject bắt đầu bằng `Fix`/`Hotfix`/`Bugfix`/`Sửa` (không theo dạng conventional) cũng là commit sửa lỗi, nên cần `Bug:`/`No-Guard:`. Trước đây luật chỉ xét subject `fix(...):`, nên `fix.`, `no message`, `Fix crash on open` đều lọt. Sự cố gốc (Goods 2026-09-25): commit `fix.` đưa phần R5/R6 làm dở vào HEAD, làm 33 dòng checklist đỏ. Dòng `Bug:` phải là id (`BUG-…`, `ABC-12`, `#123`), không nhận câu mô tả. Test: `tests/test_commit_hygiene.sh`.
- **Luật `commit-msg` được nối vào hook riêng của project** thay vì chỉ in hướng dẫn (GeelyEx2 có sẵn `.githooks/commit-msg`, nên luật chưa từng chạy ở đó). `agent-kit githooks install` chèn MỘT dòng sau `#!` của hook shell, có marker `universal-agent-devkit-chain:commit-msg`. Dòng này gọi `<repo>/.agents/devkit/…`, nên hook được git track vẫn chạy đúng trên mọi máy và không làm gì ở máy chưa cài DevKit. Hook là symlink (tới file được track) thì dòng được chèn vào file đích, symlink và quyền file giữ nguyên. Cài lại không chèn hai lần; `uninstall` chỉ gỡ dòng đó; hook không phải shell thì giữ nguyên và in lệnh. `githooks status` giờ báo cả `commit-msg`. Test: `tests/test_commit_hygiene.sh`.
- **Nhật ký checklist theo dõi `on_car` và `repeat_proof`**: một lần ghi ngoài DevKit làm mất cờ đo-trên-xe (bug sẽ âm thầm từ NEEDS_CAR thành PASS) hoặc làm cũ bằng chứng chạy lặp giờ được `losses()` báo như rollback; `checklist restore` đưa lại cờ và bản `repeat_proof` mới hơn. Test: `tests/test_repeat_proof.sh`.
- **BUG_CAPTURE bớt ghi nhầm**: vế người dùng tự nói không phải báo lỗi ("ko hỏi vấn đề…", "không phải bug", "không phải lỗi…", tới dấu câu kế tiếp) được bỏ trước khi tìm dấu hiệu lỗi. Vì vậy "20 dòng không phải bug" không thành dòng REPORTED, còn "app bị crash khi mở PDF, không phải lỗi mạng" vẫn được ghi. "ko bị" được coi như "không bị". "treo" và "màn hình đen" thành dấu hiệu lỗi; trên 6158 prompt thật, hai từ này không làm thêm dòng ghi nào. Đo trên 6158 prompt thật trong 30 ngày: luật mới chỉ khớp 2 prompt người dùng, cả 2 đều không phải báo lỗi. Không loại "bị gì": 2 câu hỏi điều tra thật ("bản 1.0.67 khách cài đang bị gì", "log báo xe bị gì?") vẫn được ghi. Test: `tests/test_bug_capture.sh`.

- **`test_evidence_gate` nhận ra lần ĐỎ Gradle có tên test dài** (GeelyEx2 2026-09-26): `RED_GRADLE` chỉ cho tối đa 80 ký tự giữa tên lớp `…Test` và `FAILED`, nên dòng `QuickInstallCapNhatQuaAliasTest > goi alias … khong so duoc FAILED` (tên hàm tiếng Việt / backtick) không khớp và hook chặn "chưa từng đỏ" dù đã đỏ. Sau ` > ` khoảng cách giờ tới hết dòng; vẫn không vượt qua xuống dòng. Test: `tests/test_legacy_lint_and_red_gradle.sh`.
- **Linter AST của post-fix-gate (Compose stability, Unity GC) không còn chặn lỗi có sẵn trong file bị chạm**: như các lớp regex (`split_new`), linter chạy cả trên bản HEAD của file; phát hiện đã có ở đó (so theo nội dung, bỏ "Line N" nên dòng dời lên/xuống vẫn là cũ) chỉ cảnh báo "đã có sẵn trong HEAD", phát hiện mới vẫn chặn. Linter lỗi giờ được log thay vì bị nuốt. Test: `tests/test_legacy_lint_and_red_gradle.sh`.

- **Commit `fix(...)` sửa code nguồn phải nêu bug nó sửa** — hook git `commit-msg` mới (`scripts/git-commit-msg.sh` → `post-fix-gate.py --commit-msg`, cài bằng `agent-kit githooks install`). Subject `fix|hotfix|bugfix(...)` có file code nguồn của profile (không tính test và file dưới `docs/`, `.agents/`, `.claude/`) cần dòng `Bug: <id>` hoặc `No-Guard: <lý do>`; project có `.agents/local/guards.json` hoặc checklist thì id phải có trong đó (không có cả hai thì nhận mọi chữ sau `Bug:` — `ponytail:` kiểm dạng id khi cần). Hook `commit-msg` riêng của project (GeelyEx2 có sẵn) được giữ nguyên, lệnh in cách chèn luật DevKit vào. Sự cố gốc (GeelyEx2 25–26/09): ~12 bug trên xe được sửa mà không thêm guard nào — luật chỉ nằm trong AGENTS.md. Test: `tests/test_commit_hygiene.sh`.
- **Pre-commit chặn file lớn và video / gói nhị phân**: file đã stage lớn hơn 5 MB (`DEVKIT_MAX_STAGED_MB`) hoặc `.mp4 .mov .zip .apk .aab .ipa .hprof …` (trừ file Git LFS). `DEVKIT_ALLOW_LARGE=1` khi người dùng yêu cầu; `DEVKIT_MAX_STAGED_MB` không phải số thì cảnh báo và dùng 5. Sự cố gốc (GeelyEx2 c4769097): video 14 MB `bugs/bug.mp4` của phiên khác lọt vào commit. Kèm luật trong `rules/essentials.md`: checkout có phiên khác dùng chung thì commit đúng đường dẫn của mình (`git commit -m … -- <paths>`), không `git add -A`/`commit -a`. Test: `tests/test_commit_hygiene.sh`.
- **Profile automotive: `hardware_safety_gate` chặn `adb logcat` chạy stream về máy tính** (`adb logcat`, `adb shell logcat`, kể cả pipe / redirect ra file trên máy / `timeout`), vì ADB Wi-Fi tới xe rớt là log chết theo (GeelyEx2 INSTINCT-099). Được phép: lệnh tự kết thúc (`-d -c -g -t<N> -L -G -S`; giá trị dính cờ như `-vtime`, `-bcrash` không tính), `logcat` chỉ là đối số (`pkill logcat`, `pidof logcat`), hoặc logcat tách rời ghi ngay trên xe (`adb shell "nohup logcat -f /data/local/tmp/x.txt &"`, `adb shell "logcat > /data/local/tmp/x.log &"`). Thông báo chặn chỉ cách làm đúng. Profile đọc từ `<project>/.agents/active-profile.json`; profile khác (hoặc không đọc được) không đổi gì; tool replicant-mcp không bị xét luật này. Luật chữ: `profiles/automotive/rules/automotive-rules.md` §8, skill `android-real-device-qa`. Test: `hooks/tests/hook_contract_test.sh` (32 ca logcat).
- **Bug đo trên xe chỉ PASS khi có lần chạy lặp sau bản sửa** (`bin/regression_checklist.py`). Bug đánh dấu `agent-kit bugs add … --on-car` (hoặc đã nộp một lần chạy lặp) có test xanh + RED-proof vẫn là `NEEDS_CAR` cho tới khi `agent-kit bugs repeat <ID> <ket-qua.json> [--scenario S]` nộp kết quả của `scripts/tools/xe-chay-lap.py` (GeelyEx2) mới hơn RED-proof, ≥3 lượt đạt và 0 lượt hỏng; lượt "không đo được" (adb rớt) không tính. Lệnh thoát 1 khi chưa đủ. Nhật ký checklist (`losses`/`restore`) chưa theo dõi `on_car`/`repeat_proof`. `ponytail:` mốc "sau bản sửa" là thời điểm RED-proof, không phải commit sửa code sau đó — nâng lên khi cần gắn với commit. Sự cố gốc: chủ xe "tự test tay 5–6 lần liên tục vẫn còn fail" sau khi bug được báo xong. Test: `tests/test_repeat_proof.sh`.
- **Luật telemetry cho luồng tự động** (chữ, không có hook): skill `observability-instrumentation` (Telemetry Contract mục 5) và `fixbugs` (Bước 1.5) — luồng chạy không người nhìn ghi signal cho cả đường thành công lẫn mọi đường thất bại, mỗi đường một vân tay grep được, để công cụ chạy lặp chấm đạt/hỏng từ log.

- **`post-fix-gate --run-tests --full` chạy lại mọi suite STALE của checklist**, không chỉ suite mà diff chạm tới (GeelyEx2 2026-09-26: `--full` in PASS "2/2" trong khi checklist còn 143 dòng bug "CẦN CHẠY LẠI", chủ xe test tay vẫn fail). Suite STALE nay fail thì gate REJECT, pass thì dòng chuyển PASS; suite STALE của phần khác không chạy được trên máy này (thiếu thiết bị/công cụ) được nêu tên, vẫn STALE và không tính vào kết luận. `--record-lesson` chỉ gắn bug bài học vào suite của thay đổi. Chưa làm: dòng FAIL/NOT_RUN không được chạy lại. Không có `--full` (Stop hook, chạy nhanh) thì không đổi. Việc "không có test hồi quy khớp thay đổi" vẫn chỉ xét suite của diff. Test: `tests/test_gate_full_stale.sh`.
- **`proof-capture.py` không chụp màn hình đang tắt, và loại ảnh gần như một màu.** Trước khi chụp, lệnh đọc `mWakefulness` từ `dumpsys power` (chỉ đọc): Asleep, Dozing hoặc Dreaming thì thoát 1, không tạo PNG. Không đọc được (không có dumpsys) thì vẫn chụp. Sau khi chụp, ảnh có dưới 1% dòng (đã lọc PNG) khác nhau bị xoá và thoát 1 (`ponytail:` đếm dòng đã lọc — nền gradient lọc "Up" gộp thành một dòng, nên ngưỡng để thấp; bỏ lọc nếu có màn thật bị từ chối). Sự cố gốc (GeelyEx2 2026-09-26): xe báo `mWakefulness=Asleep`, Display OFF, và ảnh "nghiệm thu" đen toàn bộ, trùng SHA với ảnh đen ngày 24/09. Đo trên 58 ảnh proof thật của 3 project: chỉ 2 ảnh bị loại, cả 2 đen hoàn toàn (0,1% và 0,2%); ảnh thật thấp nhất 10,6%; màn gradient + một nhãn nhỏ (2,3%) được nhận. `dumpsys` ra byte không phải UTF-8 không làm lệnh sập. Test: `tests/test_proof_capture_screen.sh`.
