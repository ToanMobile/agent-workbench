...
- **Ảnh proof chỉ miễn khi thay đổi chắc chắn không lên màn hình.** `proof_gate.sh` hỏi `bin/tree_fp.py image_required`: mặc định CẦN ảnh; chỉ miễn khi profile là `backend` lúc đầu lượt (đổi profile trong lượt không miễn gì), hoặc MỌI file đổi so với HEAD lúc đầu lượt (qua reflog: commit, merge, pull, reset đều tính; file mới; đổi tên thấy cả hai đường dẫn) nằm dưới `.agents/ .claude/ .gemini/ .github/ .githooks/ .codebase-memory/ docs/ reports/ scripts/ bin/ tools/` ở gốc, thư mục test ở gốc hoặc `src/<test source set>/`, hoặc là Markdown/LICENSE ở gốc (Markdown sâu hơn có thể là nội dung web). PNG được trích luôn bị kiểm: giờ trong tên phải thuộc lượt này và không trùng byte với ảnh proof khác (chặn `touch`/`cp` ảnh cũ). XONG luôn cần gate `--full` exit 0. Trước đây mọi lượt sửa tooling/backend chỉ có hai lối: CHƯA XONG mãi hoặc XONG sai luật.
