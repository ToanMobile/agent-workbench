...
- **What is blocked:** destructive git commands (`git push --force`, `git reset --hard`, including wrapped forms like `(…)`, `timeout`, `sudo -u`, aliases), destructive device commands (`adb remount`, `fastboot flash`, `dd of=/dev/…`), edits to files not read first, sensitive edits without a security review, and writes into the main checkout from an agent that works in a git worktree (`worktree_guard.sh`; `DEVKIT_WORKTREE`, `WORKTREE_GUARD=0`).
...
- **No silent rollback.** Every DevKit write of `.agents/regression_status.json` is journaled in `.agents/regression_journal/` (time, writer + pid, content hash; the last 500 lines and 10 snapshots, git-ignored). If the file is overwritten outside the DevKit and loses content, for example `git show HEAD:… > …` wiping red_proof results or unlinks, every load prints a warning, and so do the SessionStart context and `agent-kit health`. The warning names the last good snapshot and stays until `agent-kit checklist restore` merges that snapshot back. The merge keeps the newer red_proof and result per row by time, takes the union of links minus the recorded unlinks, and keeps rows added since; it never copies the snapshot over the file. `restore --list` lists the snapshots, `restore <snapshot>` merges a chosen one, and `restore --dismiss` keeps a deliberate reset. Caps: `CHECKLIST_JOURNAL_MAX`, `CHECKLIST_SNAPSHOTS`.
